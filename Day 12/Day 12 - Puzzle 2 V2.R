...
# Find all starting coords
x_S <- NULL
y_S <- NULL
for(r in 1:nrow(dat_grid)){
  for(c in 1:ncol(dat_grid)){
    if(dat_grid[r,c] == "S" | dat_grid[r,c] == "a"){
      x_S <- c(x_S,c)
      y_S <- c(y_S,r)
    }else{
      # Do nothing
...
dat_path <- rbind(dat_path, data.frame(x_S, y_S, x_S, y_S,""))
...
check_moves <- function(x,y){
  ...
  for(d in c("U","D","L","R")){
    ...
    start_height <- ifelse(any(x == x_S) & any(y == y_S), 1, match(dat_grid[y,x], letters))
...
repeat{
  ...
  for(i in 1:nrow(df)){
    ...
    possible_moves <- check_moves(df[i,"end_x"], df[i,"end_y"])
    ...
    if(length(possible_moves$direction) == 0){
      ...
      for(v in 1:length(possible_moves$direction)){
        ...
        end_coords <- rbind(end_coords,ec) %>% unique
